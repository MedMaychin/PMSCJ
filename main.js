...
let dataPro; //array to save object inside 
...
submit.onclick = function () {
    ...
    if (title.value != '' && price.value != '' && taxes.value != '' && ads.value != '' && discount.value != '' && category.value != '' && newPro.count <100) {

        if (mood === 'create') {
            //========================create some product or one product
            if (newPro.count > 1) {
...
function showData() {
    ...
    for (let i = 1; i < dataPro.length; i++) {
        //======================== read and count and delete data
        table += `
            <tr>
                <td>        ${i}                     </td>
